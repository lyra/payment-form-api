...
class Request
{
    ...
    /**
     * The fields to send to the payment platform.
     *
    ...
    public function __construct($encoding = 'UTF-8')
    {
        ...
        $this->addField('vads_available_languages', 'Available languages', '#^(|[A-Za-z]{2}(;[A-Za-z]{2})*)$#u');
        ...
        $this->addField('vads_currency', 'Currency', '#^\d{3}$#u', true);
        ...
        $this->addField('vads_cust_cell_phone', 'Customer cell phone', $an63);
        $this->addField('vads_cust_city', 'Customer city', '#^' . $ans . '{0,63}$#u');
        $this->addField('vads_cust_country', 'Customer country', '#^[A-Za-z]{2}$#u');
        $this->addField('vads_cust_email', 'Customer email', $regex_mail);
        $this->addField('vads_cust_first_name', 'Customer first name', $an63);
        $this->addField('vads_cust_id', 'Customer id', $an63);
        $this->addField('vads_cust_last_name', 'Customer last name', $an63);
        $this->addField('vads_cust_legal_name', 'Customer legal name', '#^' . $ans . '{0,100}$#u');
        $this->addField('vads_cust_name', 'Customer name', $ans127);
        $this->addField('vads_cust_phone', 'Customer phone', $an63);
        $this->addField('vads_cust_state', 'Customer state/region', '#^' . $ans . '{0,63}$#u');
        $this->addField('vads_cust_status', 'Customer status (private or company)', '#^PRIVATE|COMPANY$#u');
        $this->addField('vads_cust_title', 'Customer title', '#^' . $ans . '{0,63}$#u');
        $this->addField('vads_cust_zip', 'Customer zip code', $an63);
        ...
        $this->addField('vads_identifier', 'Identifier', '#^' . $ans . '{0,50}$#u');
        $this->addField('vads_insurance_amount', 'The amount of insurance', '#^' . $supzero . '$#u');
        $this->addField('vads_language', 'Language', '#^[A-Za-z]{2}$#u');
        $this->addField('vads_nb_products', 'Number of products', '#^' . $supzero . '$#u');
        $this->addField('vads_order_id', 'Order id', '#^[A-za-z0-9]{0,12}$#u');
        ...
        $this->addField('vads_payment_cards', 'Payment cards', '#^([A-Za-z0-9\-_]+;)*[A-Za-z0-9\-_]*$#u');
        $this->addField('vads_payment_config', 'Payment config', $regex_payment_cfg, true);
        $this->addField('vads_payment_src', 'Payment source', '#^$#u');
        $this->addField('vads_redirect_error_message', 'Redirection error message', $ans255);
        $this->addField('vads_redirect_error_timeout', 'Redirection error timeout', $ans255);
        $this->addField('vads_redirect_success_message', 'Redirection success message', $ans255);
        $this->addField('vads_redirect_success_timeout', 'Redirection success timeout', $ans255);
        $this->addField('vads_return_get_params', 'GET return parameters', $regex_params);
        $this->addField('vads_return_mode', 'Return mode', '#^NONE|GET|POST$#u');
        $this->addField('vads_return_post_params', 'POST return parameters', $regex_params);
        $this->addField('vads_ship_to_city', 'Shipping city', '#^' . $ans . '{0,63}$#u');
        $this->addField('vads_ship_to_country', 'Shipping country', '#^[A-Za-z]{2}$#u');
        $this->addField('vads_ship_to_delay', 'Delay of shipping', '#^INFERIOR_EQUALS|SUPERIOR|IMMEDIATE|ALWAYS$#u');
        $this->addField('vads_ship_to_delivery_company_name', 'Name of the delivery company', $ans127);
        $this->addField('vads_ship_to_first_name', 'Shipping first name', $an63);
        $this->addField('vads_ship_to_last_name', 'Shipping last name', $an63);
        $this->addField('vads_ship_to_legal_name', 'Shipping legal name', '#^' . $ans . '{0,100}$#u');
        $this->addField('vads_ship_to_name', 'Shipping name', '#^' . $ans . '{0,127}$#u');
        $this->addField('vads_ship_to_phone_num', 'Shipping phone', $ans255);
        $this->addField('vads_ship_to_speed', 'Speed of the shipping method', '#^STANDARD|EXPRESS|PRIORITY$#u');
        $this->addField('vads_ship_to_state', 'Shipping state', $an63, false, 63);
        $this->addField('vads_ship_to_status', 'Shipping status (private or company)', '#^PRIVATE|COMPANY$#u');
        $this->addField('vads_ship_to_street', 'Shipping street', $ans127);
        $this->addField('vads_ship_to_street2', 'Shipping street (2)', $ans127);
        $this->addField('vads_ship_to_type', 'Type of the shipping method', $regex_ship_type);
        $this->addField('vads_ship_to_zip', 'Shipping zip code', $an63, false, 63);
        $this->addField('vads_shipping_amount', 'The amount of shipping', '#^' . $supzero . '$#u');
        $this->addField('vads_shop_name', 'Shop name', $ans127);
        $this->addField('vads_shop_url', 'Shop URL',
            '#^https?://(\w+(:\w*)?@)?(\S+)(:[0-9]+)?[\w\#!:.?+=&%@`~;,|!\-/]*$#u');
        $this->addField('vads_site_id', 'Shop ID', '#^\d{8}$#u', true);
        $this->addField('vads_tax_amount', 'The amount of tax', '#^' . $supzero . '$#u');
        $this->addField('vads_theme_config', 'Theme configuration', '#^[^;=]+=[^;=]*(;[^;=]+=[^;=]*)*;?$#u');
        $this->addField('vads_threeds_mpi', 'Enable / disable 3-D Secure', '#^[0-2]$#u');
        ...
        $this->addField('vads_url_cancel', 'Cancel URL', $ans127);
        $this->addField('vads_url_error', 'Error URL', $ans127);
        $this->addField('vads_url_referral', 'Referral URL', $ans127);
        $this->addField('vads_url_refused', 'Refused URL', $ans127);
        $this->addField('vads_url_return', 'Return URL', $ans127);
        $this->addField('vads_url_success', 'Success URL', $ans127);
        $this->addField('vads_user_info', 'User info', $ans255);
        $this->addField('vads_validation_mode', 'Validation mode', '#^[01]?$#u');
        $this->addField('vads_version', 'Platform version', '#^V2$#u', true);
        ...
        $timestamp = time();
        $this->set('vads_trans_id', Util::generateTransId($timestamp));
     ...
     * @return boolean
     */
    private function addField($name, $label, $regex, $required = false, $value = null)
    {
        $this->requestParameters[$name] = new Field($name, $label, $regex, $required);
    ...
    public function addProduct($label, $amount, $qty, $ref, $type = null, $vat = null)
    {
        ...
        $ok &= $this->addField('vads_product_label' . $index, 'Product label', '#^[^<>"+-]{0,255}$#u', false, $label);
        $ok &= $this->addField('vads_product_amount' . $index, 'Product amount', '#^[1-9]\d*$#u', false, $amount);
        $ok &= $this->addField('vads_product_qty' . $index, 'Product quantity', '#^[1-9]\d*$#u', false, $qty);
    ...
    {
        return $this->addField('vads_ext_info_' . $key, 'Extra info ' . $key, '#^.{0,255}$#u', false, $value);
    }
